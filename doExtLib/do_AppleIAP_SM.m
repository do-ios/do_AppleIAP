...
-(void)verifyPurchaseWithPaymentTransaction
{
    ...
    [node setObject:receiptString forKey:@"receiptData"];
    doInvokeResult *invoke = [[doInvokeResult alloc]init:self.UniqueKey];
    [invoke SetResultNode:node];
    [_scritEngine Callback:_callbackName :invoke];
...
//    NSLog(@"%@",dic);
//    if([dic[@"status"] intValue] == 0 && dic)
//    {
//        NSLog(@"购买成功！");
//        doInvokeResult *_invokeResult = [[doInvokeResult alloc] init];
//        [_invokeResult SetResultText:receiptString];
//        [_scritEngine Callback:_callbackName :_invokeResult];
